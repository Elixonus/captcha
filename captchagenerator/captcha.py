...
class Captcha:
        ...
        for i in range(size[0]):
            for j in range(size[1]):
                ...
                self.image.putpixel(
                    (i, j),
                    (
                        min(pixel[0] + round(50 * r1 * ir * jr), 255),
    ...
    def add_emoji(self, name: str) -> None:
        emoji_size = randint(int(0.1 * max(*self.size)), int(0.2 * max(*self.size)))
        emoji_center = (randint(0, self.size[0]), randint(0, self.size[1]))
        for s in self.stack:
            if hypot(emoji_center[0] - s[0], emoji_center[1] - s[1]) < 0.1 * max(
                *self.size
            ):
                self.add_emoji(name)
                return
        ...
        emoji_position = (
            emoji_center[0] - emoji_size // 2,
            emoji_center[1] - emoji_size // 2,
        )
        emoji = (
            Image.open(f"emojis/{name}.png")
